...
values
 ...
 ('site_title', '"026Newsblog"'::jsonb, v_admin, now()),
 ('site_description', '"East Africa\'s independent news platform."'::jsonb, v_admin, now()),
 ('site_url', '"https://026newsblog.com"'::jsonb, v_admin, now()),
 ('support_email', '"hello@026newsblog.com"'::jsonb, v_admin, now()),
 ('favicon_url', '""'::jsonb, v_admin, now()),
 ('enable_registration', 'true'::jsonb, v_admin, now()),
 ('enable_comments', 'true'::jsonb, v_admin, now()),
 ('enable_chat', 'true'::jsonb, v_admin, now()),
 ('require_email_verification', 'false'::jsonb, v_admin, now()),
 ('content_per_page', '20'::jsonb, v_admin, now()),
 ('breaking_news_ticker', 'true'::jsonb, v_admin, now()),
 ('auto_approve', 'false'::jsonb, v_admin, now()),
 ('enable_google_oauth', 'true'::jsonb, v_admin, now()),
 ('enable_github_oauth', 'true'::jsonb, v_admin, now()),
 ('invite_only_mode', 'false'::jsonb, v_admin, now()),
 ('default_language', '"en-US"'::jsonb, v_admin, now()),
 ('timezone', '"Africa/Nairobi"'::jsonb, v_admin, now()),
 ('date_format', '"Jul 12, 2026"'::jsonb, v_admin, now()),
 ('currency_display', '"USD with KES"'::jsonb, v_admin, now()),
 ('hero_slideshow', 'true'::jsonb, v_admin, now()),
 ('trending_sidebar', 'true'::jsonb, v_admin, now()),
 ('newsletter_widget', 'true'::jsonb, v_admin, now()),
 ('rss_feed_section', 'true'::jsonb, v_admin, now()),
 ('chat_widget', 'true'::jsonb, v_admin, now()),
 ('primary_color', '"#1a8a6e"'::jsonb, v_admin, now()),
 ('accent_color', '"#f97316"'::jsonb, v_admin, now()),
 ('hero_slideshow_interval', '5'::jsonb, v_admin, now()),
 ('default_feed_tab', '"foryou"'::jsonb, v_admin, now()),
 ('payout_processing_time', '"24h"'::jsonb, v_admin, now()),
 ('allow_self_publishing', 'true'::jsonb, v_admin, now()),
 ('moderation_external_links', 'true'::jsonb, v_admin, now()),
 ('moderation_community_reports', 'true'::jsonb, v_admin, now()),
 ('moderation_auto_hide', 'true'::jsonb, v_admin, now()),
 ('moderation_profanity_filter', 'false'::jsonb, v_admin, now()),
 ('moderation_ai_threshold', '80'::jsonb, v_admin, now()),
 ('min_word_count', '300'::jsonb, v_admin, now()),
 ('max_tags', '8'::jsonb, v_admin, now()),
 ('auto_generate_audio', 'true'::jsonb, v_admin, now()),
 ('reading_time_display', 'true'::jsonb, v_admin, now()),
 ('related_articles', 'true'::jsonb, v_admin, now()),
 ('seo_title_template', '"{title} · 026Newsblog"'::jsonb, v_admin, now()),
 ('seo_description', '"026Newsblog is East Africa\'s creator-first news platform."'::jsonb, v_admin, now()),
 ('social_twitter', '"@026newsblog"'::jsonb, v_admin, now()),
 ('social_facebook', '"https://facebook.com/026newsblog"'::jsonb, v_admin, now()),
 ('social_linkedin', '"https://linkedin.com/company/026newsblog"'::jsonb, v_admin, now()),
 ('social_instagram', '"@026newsblog"'::jsonb, v_admin, now()),
 ('sitemap_auto_generate', 'true'::jsonb, v_admin, now()),
 ('index_articles', 'true'::jsonb, v_admin, now()),
 ('index_authors', 'true'::jsonb, v_admin, now()),
 ('feed_cache_ttl', '300'::jsonb, v_admin, now()),
 ('article_cache_ttl', '3600'::jsonb, v_admin, now()),
 ('rate_limiting', 'true'::jsonb, v_admin, now()),
 ('cors_restriction', 'true'::jsonb, v_admin, now()),
 ('brute_force_protection', 'true'::jsonb, v_admin, now()),
 ('force_https', 'true'::jsonb, v_admin, now()),
 ('backup_frequency', '"daily"'::jsonb, v_admin, now()),
 ('backup_retention', '"30"'::jsonb, v_admin, now()),
 ('ad_sponsored_article', '500'::jsonb, v_admin, now()),
 ('ad_display_ads', '200'::jsonb, v_admin, now()),
 ('ad_newsletter_sponsor', '150'::jsonb, v_admin, now()),
 ('ad_homepage_feature', '800'::jsonb, v_admin, now()),
 ('storage_bucket', '"026newsblog-uploads"'::jsonb, v_admin, now()),
 ('storage_max_upload', '10'::jsonb, v_admin, now()),
 ('tts_voice_model', '"en-US-Neural2-D"'::jsonb, v_admin, now()),
 ('mpesa_shortcode', '"174379"'::jsonb, v_admin, now()),
 ('mpesa_environment', '"production"'::jsonb, v_admin, now()),
 ('email_from', '"noreply@026newsblog.com"'::jsonb, v_admin, now()),
 ('email_reply_to', '"hello@026newsblog.com"'::jsonb, v_admin, now());
