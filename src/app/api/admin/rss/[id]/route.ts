...
export const GET = requireRole('admin', async (req) => {
  ...
  const { count: importedAsArticles } = await sb
    .from('articles')
    .select('*', { count: 'exact', head: true })
    .eq('source_url', feed.url);
  ...
  return NextResponse.json({
    feed: {
      ...
      stats: {
        ...
        importedAsArticles: importedAsArticles || 0,
...
export const POST = requireRole('admin', async (req) => {
  ...
  if (action === 'import') {
    ...
    if (!items?.length) {
      return NextResponse.json({ imported: 0, message: 'No items to import' });
    }
