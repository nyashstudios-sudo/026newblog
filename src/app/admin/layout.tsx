'use client';

import { Suspense } from 'react';
import Link from 'next/link';
import { usePathname, useSearchParams } from 'next/navigation';
import { useAuth } from '@/components/providers/auth-provider';

interface NavItem { href: string; label: string; icon: string; badge?: string | number }

const sidebarSections: { label: string; items: NavItem[] }[] = [
  {
    label: 'Platform',
    items: [
      { href: '/admin', label: 'Overview', icon: 'M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z' },
      { href: '/admin/analytics', label: 'Analytics', icon: 'M22 12h-4l-3 9L9 3l-3 9H2' },
    ],
  },
  {
    label: 'Users',
    items: [
      { href: '/admin/authors', label: 'Authors', icon: 'M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2 M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8z M23 21v-2a4 4 0 0 0-3-3.87 M16 3.13a4 4 0 0 1 0 7.75' },
      { href: '/admin/readers', label: 'Readers', icon: 'M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2 M9 7a4 4 0 1 0 0-8 4 4 0 0 0 0 8z' },
      { href: '/admin/authors?filter=pending', label: 'Applications', icon: 'M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2 M8.5 7a4 4 0 1 0 0-8 4 4 0 0 0 0 8z M20 8v6 M23 11h-6', badge: '5' },
    ],
  },
  {
    label: 'Content',
    items: [
      { href: '/admin/articles', label: 'All Articles', icon: 'M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z M14 2v6h6' },
      { href: '/admin/articles/write', label: 'Write / Publish', icon: 'M12 20h9 M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z' },
      { href: '/admin/rss', label: 'RSS Feeds', icon: 'M4 11a9 9 0 0 1 9 9 M4 4a16 16 0 0 1 16 16 M5 20a1 1 0 1 0 0-2 1 1 0 0 0 0 2z' },
      { href: '/admin/moderation', label: 'Moderation', icon: 'M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z' },
    ],
  },
  {
    label: 'System',
    items: [
      { href: '/admin/security', label: 'Security', icon: 'M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z' },
      { href: '/admin/settings', label: 'Settings', icon: 'M12 20h9 M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z' },
      { href: '/admin/contact', label: 'Contact Messages', icon: 'M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z', badge: 0 },
    ],
  },
];

function getActive(pathname: string, searchParams: URLSearchParams, item: NavItem): boolean {
  const basePath = item.href.split('?')[0];
  const itemFilter = item.href.split('?')[1];
  if (!itemFilter) return pathname === basePath;
  const filterParam = itemFilter.split('=')[1];
  return pathname === basePath && searchParams.get('filter') === filterParam;
}

function Sidebar() {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const { user } = useAuth();
  const initials = user ? `${user.firstName[0]}${user.lastName[0]}` : 'A';

  return (
    <aside className="dash-sidebar">
      <Link href="/" className="dash-sidebar-logo"><span>026</span>Newsblog</Link>
      <div className="dash-sidebar-role">Admin Panel</div>
      <div className="dash-sidebar-section">
        <nav className="dash-sidebar-nav">
          <Link href="/" className="dash-sidebar-link">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" /><polyline points="9 22 9 12 15 12 15 22" /></svg>
            Public Home
          </Link>
        </nav>
      </div>
      {sidebarSections.map(section => (
        <div key={section.label} className="dash-sidebar-section">
          <div className="dash-sidebar-label">{section.label}</div>
          <nav className="dash-sidebar-nav">
            {section.items.map(item => {
              const active = getActive(pathname, searchParams, item);
              return (
                <Link key={item.href} href={item.href} className={`dash-sidebar-link${active ? ' active' : ''}`}>
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d={item.icon} /></svg>
                  {item.label}
                  {item.badge != null && <span className="sidebar-badge">{item.badge}</span>}
                </Link>
              );
            })}
          </nav>
        </div>
      ))}
      {user && <div className="dash-sidebar-footer">
        <Link href={`/profile/${user.username}`} className="dash-sidebar-profile" style={{ textDecoration: 'none' }}>
          <div className="dash-sidebar-avatar">{initials}</div>
          <div className="dash-sidebar-profile-info">
            <div className="dash-sidebar-profile-name">{user.firstName} {user.lastName}</div>
            <div className="dash-sidebar-profile-role">{user.role}</div>
          </div>
        </Link>
      </div>}
    </aside>
  );
}

function SidebarFallback() {
  return (
    <aside className="dash-sidebar">
      <div className="dash-sidebar-logo"><span>026</span>Newsblog</div>
      <div className="dash-sidebar-role">Admin Panel</div>
      {[1, 2, 3, 4].map(s => (
        <div key={s} className="dash-sidebar-section">
          <div style={{ height: 12, width: 60, background: 'var(--border-subtle)', borderRadius: 4, marginBottom: 8 }} />
          {[1, 2].map(i => (
            <div key={i} style={{ height: 32, background: 'var(--border-subtle)', borderRadius: 8, marginBottom: 4 }} />
          ))}
        </div>
      ))}
    </aside>
  );
}

export default function AdminLayout({ children }: { children: React.ReactNode }) {
  const { loading } = useAuth();
  if (loading) return null;

  return (
    <div className="dash-layout">
      <Suspense fallback={<SidebarFallback />}>
        <Sidebar />
      </Suspense>
      <main className="dash-main">
        {children}
      </main>
    </div>
  );
}
