...
export default function AdminModerationPage() {
  ...
  return (
    <>
      ...
      {/* Filter Tabs */}
      <div style={{
        display: 'flex', gap: 4, marginBottom: 24, background: 'var(--bg-surface)',
        padding: 4, borderRadius: 10, border: '1px solid var(--border-subtle)', width: 'fit-content',
      }}>
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              onMouseEnter={(e) => { if (!isActive) { e.currentTarget.style.color = 'var(--text-primary)'; e.currentTarget.style.background = 'var(--bg-inset)'; } }}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {filteredItems.map((item) => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
              ...
              <span style={{ fontSize: '0.72rem', color: 'var(--text-tertiary)', flex: 1 }}>{item.reason || 'Flagged for review'}</span>
              <span style={{ fontSize: '0.7rem', color: 'var(--text-tertiary)', whiteSpace: 'nowrap' }}>{formatRelativeDate(item.createdAt)}</span>
            </div>
        ...
        {filteredItems.length === 0 && (
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', textAlign: 'center', padding: 48 }}>No items in this queue.</p>
        )}
      </div>
    </>
  );
