...
export default function RegisterPage() {
  ...
  useEffect(() => {
    if (settings.enableRegistration === false) router.replace('/auth/login');
    else setGateChecked(true);
  }, [settings.enableRegistration, router]);
  ...
  const strength = pwLen >= 12 ? 4 : pwLen >= 8 ? 3 : pwLen >= 4 ? 2 : pwLen > 0 ? 1 : 0;
    ...
    <div className="auth-layout">
      ...
      <div className="auth-form-panel">
        ...
        <div className="auth-form-wrap">
          ...
          {(settings.enableGoogleOauth !== false || settings.enableGithubOauth !== false) && (
            <div className="oauth-buttons">
              {settings.enableGoogleOauth !== false && (
              ...
              {settings.enableGithubOauth !== false && (
                <button className="oauth-btn">
              ...
              <div className="field-row">
                <div className="field">
                  ...
                  <input
                    ...
                    className="field-input" placeholder="Wayne" required
                  />
                </div>
                <div className="field">
                  ...
                  <input
                    ...
                    className="field-input" placeholder="Nyamu" required
                  />
                </div>
              </div>
              <div className="field">
                <label className="field-label">Email</label>
                <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="field-input" placeholder="wayne@example.com" required />
              </div>
              <div className="field">
                <label className="field-label">Password</label>
                <div className="field-input-wrap">
                  <input type={showPassword ? 'text' : 'password'} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className="field-input" placeholder="Min. 8 characters" required minLength={8} />
                ...
                <div className="password-strength">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className={`strength-bar${i <= strength ? ` ${strength >= 3 ? 'active' : strength === 2 ? 'medium' : 'weak'}` : ''}`} />
                  ))}
                </div>
                <span className="field-helper">Use a mix of letters, numbers, and symbols</span>
              </div>
            </div>
