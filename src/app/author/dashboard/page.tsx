...
export default function AuthorDashboardPage() {
  ...
  const [totals, setTotals] = useState({ views: 0, likes: 0, comments: 0, followers: 0 });
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch('/api/author/activity').then(r => r.json()).catch(() => null),
    ]).then(([analytics, arts, act]) => {
      if (analytics) {
        setTotals({ ...(analytics.totals || { views: 0, likes: 0, comments: 0, shares: 0 }), followers: analytics.followers || 0 });
        setDailyViews(analytics.dailyViews || []);
        setEarnings(Number(analytics.monthlyEarnings || 0));
      }
      if (act?.activity) {
  ...
  const mockActivity = activity.map((a) => {
    let text: any = null;
    if (a.type === 'like') text = <><strong>{a.actorName}</strong> liked your article &ldquo;{a.articleTitle}&rdquo;</>;
    else if (a.type === 'comment') text = <><strong>{a.actorName}</strong> commented: &ldquo;{a.excerpt}&rdquo;</>;
    else if (a.type === 'follow') text = <><strong>{a.actorName}</strong> started following you</>;
    else if (a.type === 'earn') text = <>You earned <strong>${Number(a.amountUsd).toFixed(2)}</strong> from &ldquo;{a.articleTitle}&rdquo;</>;
    return { type: a.type, text, time: timeAgo(a.createdAt) };
  });
  ...
  const statCards = [
    ...
    { label: 'Total Earnings', value: earnings >= 1000 ? '$' + (earnings / 1000).toFixed(1) + 'K' : '$' + earnings.toFixed(2), change: null, up: true },
    { label: 'Articles Published', value: publishedCount.toString(), change: null, up: true },
    { label: 'Total Followers', value: totals.followers >= 1000 ? `${(totals.followers / 1000).toFixed(1)}K` : totals.followers.toString(), change: null, up: true },
  ];
